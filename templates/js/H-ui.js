...
!function($) {
	$.fn.HuitabSpecial = function(options){
		var defaults = {
			tabBar:'.tabBarSpecial span',
			tabCon:".tabConSpecial",
			className:"current",
			tabEvent:"click",
//			index:0,
		}
		...
		this.each(function(){
			...
			that.find(options.tabBar).on(options.tabEvent,function(){
				var index = that.find(options.tabBar).index(this);
				if(that.find(options.tabBar).eq(index).attr("class")=="current")
				{
					that.find(options.tabBar).eq(index).removeClass(options.className);
					that.find(options.tabCon).eq(index).hide();
				}
				else{
					that.find(options.tabBar).removeClass(options.className);
				$(this).addClass(options.className);
